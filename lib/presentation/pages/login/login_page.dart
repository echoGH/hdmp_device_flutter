...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(loginProvider.notifier).getCustomers();
    });
  }
  ...
  void _showCustomerPicker() {
    ...
    if (state.customers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('暂无机构列表')));
      return;
    }
          ...
          child: Column(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  border: Border(bottom: BorderSide(color: AppColors.lineBg, width: 1)),
                ),
                child: Text(
                  '请选择机构',
                  style: TextStyle(fontSize: AppDimens.sp16, fontWeight: FontWeight.bold, color: AppColors.color333333),
                ),
              ),
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = state.customerId == customer.customerId.toString();
                    return InkWell(
                      onTap: () {
                        ref.read(loginProvider.notifier).selectCustomer(customer.customerId.toString());
                        Navigator.of(context).pop();
                      },
                      child: Container(
                        padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 16.h),
                        decoration: BoxDecoration(
                          color: isSelected ? AppColors.clickBgBlue : Colors.transparent,
                          border: Border(bottom: BorderSide(color: AppColors.lineBg, width: 1)),
                        ),
                        child: Row(
                          children: [
                            ...
                            if (isSelected) Icon(Icons.check, color: AppColors.colorPrimary, size: 20.r),
                          ],
                        ),
                      ),
                    );
            ...
            Positioned(
              ...
              child: Image.asset(
                ...
                errorBuilder: (context, error, stackTrace) {
                  ...
                  return Container(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [AppColors.colorPrimary, AppColors.colorPrimary.withOpacity(0.8)]),
                    ),
                  );
                },
              ),
            ...
            SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  GestureDetector(
                    onTap: _handleTouch,
                    child: Container(height: 200.h, color: Colors.transparent),
                  ),
                ],
              ),
            ),
        ...
        child: Row(
          children: [
            Expanded(
              child: Text(
                '机构',
                style: TextStyle(fontSize: AppDimens.sp14, color: AppColors.color999999),
              ),
            ),
  ...
  Widget _buildUsernameInput() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.person, size: 20.r, color: AppColors.color999999),
          SizedBox(width: 8.w),
          Expanded(
            child: TextField(
          ...
          if (_usernameController.text.isNotEmpty)
            GestureDetector(
              ...
              child: Icon(Icons.clear, size: 18.r, color: AppColors.color999999),
            ),
        ],
      ),
    );
  ...
  Widget _buildPasswordInput() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.lock, size: 20.r, color: AppColors.color999999),
          SizedBox(width: 8.w),
          Expanded(
            child: TextField(
