...
/// 登录Provider（完全沿用Android项目的逻辑）
class LoginNotifierNew extends Notifier<LoginStateNew> {
  late LoginService _loginService;

  @override
  LoginStateNew build() {
    _loginService = LoginService(DioClient.instance.dio);
    return const LoginStateNew();
  }
    ...
    try {
      ...
      if (response.success == true && response.data != null) {
        state = state.copyWith(
          ...
          customerId: response.data!.isNotEmpty
              ? response.data!.first.customerId.toString()
              : '',
        );
      } else {
        state = state.copyWith(
          isLoading: false,
          errorMessage: response.message ?? '获取客户列表失败',
        );
      }
    } catch (e) {
      state = state.copyWith(
        isLoading: false,
        errorMessage: '获取客户列表异常: $e',
      );
    }
  }
  ...
  /// 执行登录（完全沿用Android项目的逻辑）
  Future<bool> login(String username, String password) async {
    // 验证输入
    if (username.trim().isEmpty) {
      state = state.copyWith(errorMessage: 'UName can\'t be empty');
      return false;
    }
...
/// 登录Provider实例
final loginProviderNew = NotifierProvider<LoginNotifierNew, LoginStateNew>(() {
  return LoginNotifierNew();
});
